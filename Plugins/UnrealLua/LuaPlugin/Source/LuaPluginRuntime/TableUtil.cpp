...
void UTableUtil::call(lua_State*inL, FFrame& Stack, RESULT_DECL)
{
	...
	else
	{
		...
		{
			UTableUtil::popproperty(inL, -LuaStackIndex, ReturnProp, RESULT_PARAM);
			++LuaStackIndex;
		}

		for (auto OutParmRec : OutParmsArr)
		{
			UTableUtil::popproperty(inL, -LuaStackIndex, OutParmRec->Property, OutParmRec->PropAddr);
			++LuaStackIndex;
		}
...
void UTableUtil::push_totable(lua_State*inL, UMapProperty* Property, const void* p)
{
	...
	{
		uint8* PairPtr = result.GetPairPtr(i);
		UTableUtil::push_totable(inL, Property->KeyProp, PairPtr + Property->MapLayout.KeyOffset);
